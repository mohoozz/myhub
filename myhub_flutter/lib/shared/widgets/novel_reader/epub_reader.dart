...
class _EpubReaderPageState extends ConsumerState<EpubReaderPage> {
  ...
  ///
  /// 同步记录 chapter + page：chapter > 0 时跳到对应章节并跳过封面页，
  /// page > 0 时记录到 [_pendingPage]，由翻页模式首帧消费跳转。
  /// [_pendingRestore] 在恢复流程期间为 true，区分恢复与用户主动切章（_goToChapter
  /// 在 _pendingRestore=true 时使用 _pendingPage 初始化页码，结束后清空标志）。
  /// 保留 [_pendingPage] 给 page_mode 通过 didUpdateWidget 消费；用户首次翻页后再清空。
  ...
  Future<void> _restoreProgress() async {
    try {
      ...
      if (p.progressJson.isNotEmpty) {
        ...
        // 翻页模式：chapter > 0 跳到章节并跳过封面页，page > 0 定位章节内页码
        if (chapter >= 0 && chapter < _toc.length) {
