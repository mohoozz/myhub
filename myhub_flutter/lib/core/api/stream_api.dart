...
abstract final class StreamApi {
  ...
  static String hlsPlaylistUrl(int sourceId, String path) {
    return '${Env.apiBaseUrl}/api/stream/hls/${hlsSessionId(sourceId, path)}/playlist.m3u8';
  }

  /// HLS 分片地址（segName 形如 seg_00000.ts）。
  static String hlsSegmentUrl(int sourceId, String path, String segName) {
    return '${Env.apiBaseUrl}/api/stream/hls/${hlsSessionId(sourceId, path)}/segment/$segName';
  }

  /// 字幕转换地址（srt/ass → vtt）。
  static String subtitleUrl(int sourceId, String path) {
    return '${Env.apiBaseUrl}/api/stream/subtitle?source=$sourceId&path=${Uri.encodeComponent(path)}';
  }
