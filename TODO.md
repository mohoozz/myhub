...
- [ ] `shared/widgets/comic_reader/next_comic_tip.dart`：下一本推荐提示
  ...
  - 同目录查找下一个漫画文件
  - 不自动打开（已去除 5 秒倒计时自动打开方案），点击提示条才打开
  - 点击提示条立即打开、点击空白区域关闭
...
- [x] 经常会退出到登录界面为什么，并且选择了记住密码，退出到登录界面后用户名和密码都清空了
- [x] 打开部分txt文件，会显示乱码，需要检查下是否是编码问题，纯txt阅读器和小说阅读器都会有这个问题（服务端编码检测补全 UTF-16 LE/BE：BOM 判定 + 无 BOM 启发式 + 码元对齐建索引 + 旧缓存编码复核自愈）
- [x] 加一个内置的浏览器（需求已拆解至第 13 章浏览器模块，F-601~F-605）
- [x] ios端，内置浏览器，打开网页，进入新网页，后退的按钮无法点击，并且无法通过左侧滑动返回上一页
- [x] ios端，内置浏览器，把操作栏，放到底部，方便操作
- [x] ios端，内置浏览器，上下滑动页面后，把操作栏缩小到屏幕底部，只有点击后才显示操作栏（任意方向滚动都收起为 0 高，仅点击页面重新展开，AnimatedSize 动画，标签切换重置展开）
- [x] 浏览器目前打开的页面没有缓存，每次打开app都是重新开始（标签会话持久化：退出时保存标签列表+激活标签到 SharedPreferences，启动恢复；并修复 WebView 随 URL 重建导致导航历史栈丢失的问题）
- [x] 浏览界面，进入一个封面完全没有加载的目录，点击视频会卡顿严重，检查下是否是拉取封面的原因
- [x] 播放音频时候，会先从零开始播放，然后再跳转到历史记录
- [x] 正在阅读界面，不会自动刷新进度，每次上报进度时候刷新一下
- [x] 浏览界面的定位到原路径功能，现在的边框提醒会一直常亮，改为呼吸灯只提示10s
- [x] PC端/移动端，文件无法下载，例如图片，txt
- [x] txt文件无法在线修改
- [ ] 视频播放增加一个纯音频播放模式
- [ ] ios端的音量调节跨度从1%改成5%
- [ ] 漫画、视频、音频、小说阅读时候，现在是否会有缓存，把缓存放到本地，方便下次打开不用再次加载，或者网络差的时候，缓缓加载
