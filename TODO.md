...
- [ ] 漫画阅读时候，把缓存图片放到本地，方便下次打开不用再次加载
- [ ] 浏览界面，进入一个封面完全没有加载的目录，点击视频会卡顿严重，检查下是否是拉取封面的原因
- [ ] 播放音频时候，会先从零开始播放，然后再跳转到历史记录
- [ ] 正在阅读界面，不会自动刷新进度，每次上报进度时候刷新一下
- [ ] 定位到原路径，现在的边框提醒会一直常亮，改为呼吸灯，只显示5s
- [ ] 文件无法下载，例如图片，txt
- [ ] txt无法在线修改
- [ ] ios端的音量调节跨度从1%改成5%
- [ ] 视频播放增加一个纯音频播放模式
