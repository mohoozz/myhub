...
- [ ] 视频播放，没有按照播放历史进度，播放到历史记录
- [ ] 音视频播放现在的退出按钮，改为直接退出，不进入mini播放器模式，单独在下方的功能栏，增加一个按钮进入mini模式。
- [ ] 移动端模式，可以从中间向下拖动，进入mini模式
- [ ] 阅读界面，增加列表显示，目前长按是显示菜单，改为进入多选模式
- [ ] 点击图片文件，进入预览界面，可以手动切换上一张、下一张
- [ ] 5.8 连续播放推荐（F-207，新增） 
