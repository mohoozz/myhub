...
- [ ] 阅读界面和浏览界面，类型的默认图标和目录图标太小了，目前是一个小图标+大白底，改成去掉白底，把图标放大，只用图标显示（列表模式和卡片模式都需要调整）
...
- [ ] 阅读界面的进度条，参考浏览界面的饼状图进度条，把进度条改为饼状图进度条，百分比进度显示在饼状图下面
- [ ] 设置里面的行数，点击选择其他行数后，设置中的数字没有变化，切换主题后才会更新，直接进入浏览界面，不会立刻生效，要跳转到其他其他目录后才会生效
- [ ] 阅读界面长按文件触发的菜单栏中，缺少定位到路径源中的位置功能，参考flutter的实现，增加一个定位到路径源中的位置功能
- [ ] 当从浏览界面删除/移动文件时候，阅读界面的记录也同步删除下；重命名文件后，阅读界面的记录也同步重命名下
- [ ] 阅读界面的多选模式，操作栏中增加删除源文件的功能
- [ ] 阅读界面长按文件触发的菜单栏，增加属性选项和删除源文件的功能
